...
class CL837RRMonitor:
    """CL837 Respiratory Rate History Monitor"""
    ...
    def __init__(self):
        ...
        self.CMD_GET_RR_RECORD = 0x24  # 36 in decimal
        self.CMD_GET_RR_DATA = 0x25    # 37 in decimal
        
        # RR data storage
        self.rr_records = []
        self.rr_data = []
        self.rr_records_complete = False
        self.rr_data_complete = False
    ...
    def notification_handler(self, sender, data):
        ...
        try:
            ...
            # Response to GET_RR_RECORD (0x24)
            if cmd == self.CMD_GET_RR_RECORD:
                self.parse_rr_record_response(data)
            
            # Response to GET_RR_DATA (0x25)
            elif cmd == self.CMD_GET_RR_DATA:
                self.parse_rr_data_response(data)
    ...
    def parse_rr_record_response(self, data):
        """Parse RR record list response"""
        try:
            if len(data) < 5:
                return
            ...
            if len(data) == 5 and data[3] == 0xFF and data[4] == 0xFF:
                self.rr_records_complete = True
                print(f"\n✓ RR records download complete ({len(self.rr_records)} records)")
                return
            ...
            if len(data) >= 7:
                ...
                self.rr_records.append({
                    'utc': utc_timestamp,
                    'datetime': datetime.utcfromtimestamp(utc_timestamp).strftime('%Y-%m-%d %H:%M:%S')
                })
                
                print(f"  RR Record: {datetime.utcfromtimestamp(utc_timestamp).strftime('%Y-%m-%d %H:%M:%S')}")
                
        except Exception as e:
            print(f"Parse RR record error: {e}")
            traceback.print_exc()

    def parse_rr_data_response(self, data):
        """Parse RR data response"""
        try:
            if len(data) < 5:
                return
            ...
            # Check if this is the end marker
            if len(data) == 5 and data[3] == 0xFF and data[4] == 0xFF:
                self.rr_data_complete = True
                print(f"\n✓ RR data download complete ({len(self.rr_data)} measurements)")
                return
            
            # Parse RR data: Based on HistoryOfRespiratoryRate.java
            # Format: [header, length, cmd, sequence, rr_value, ...]
            if len(data) >= 5:
                sequence = data[3]
                rr_value = data[4]  # Breaths per minute
                ...
                print(f"  RR Data: Seq={sequence}, Rate={rr_value} breaths/min")
                
        except Exception as e:
            print(f"Parse RR data error: {e}")
            traceback.print_exc()
    ...
    async def get_rr_records(self):
        """Request RR record list"""
        try:
            print("\nRequesting RR records...")
            self.rr_records = []
            self.rr_records_complete = False
            
            await self.send_command(self.CMD_GET_RR_RECORD, [])
            
            # Wait for records to download (max 30 seconds)
            timeout = 30
            start_time = time.time()
            
            while not self.rr_records_complete:
                await asyncio.sleep(0.1)
                if time.time() - start_time > timeout:
                    print("✗ Timeout waiting for RR records")
                    return False
            
            return True
            
        except Exception as e:
            print(f"Get RR records error: {e}")
            return False

    async def get_rr_data(self, utc_timestamp):
        """Request RR data for specific timestamp"""
        try:
            print(f"\nRequesting RR data for {datetime.utcfromtimestamp(utc_timestamp).strftime('%Y-%m-%d %H:%M:%S')}...")
            self.rr_data = []
            self.rr_data_complete = False
            ...
            await self.send_command(self.CMD_GET_RR_DATA, utc_bytes)
            
            # Wait for data to download (max 30 seconds)
            timeout = 30
            start_time = time.time()
            
            while not self.rr_data_complete:
                await asyncio.sleep(0.1)
                if time.time() - start_time > timeout:
                    print("✗ Timeout waiting for RR data")
                    return False
            
            return True
            
        except Exception as e:
            print(f"Get RR data error: {e}")
            return False

    def export_to_csv(self):
        """Export RR data to CSV file"""
        try:
            if not self.rr_records:
                print("\nNo RR data to export")
                return
            
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            filename = f"rr_history_{timestamp}.csv"
            ...
            with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                # Data rows
                for i, record in enumerate(self.rr_records, 1):
                    # Filter data for this record (if we downloaded detailed data)
                    record_data = [d for d in self.rr_data if d.get('record_utc') == record['utc']]
                    
                    if record_data:
                        rr_values = [d['respiratory_rate'] for d in record_data if d['respiratory_rate'] > 0]
                        avg_rr = sum(rr_values) / len(rr_values) if rr_values else 0
                        min_rr = min(rr_values) if rr_values else 0
                        max_rr = max(rr_values) if rr_values else 0
                        count = len(record_data)
                    else:
                        avg_rr = min_rr = max_rr = count = 0
                    ...
                    writer.writerow([
                        ...
                        record['datetime'],
                        count,
                        f"{avg_rr:.1f}" if avg_rr > 0 else "N/A",
                        min_rr if min_rr > 0 else "N/A",
                        max_rr if max_rr > 0 else "N/A"
                    ])
            
            print(f"\n✓ Data exported to: {filename}")
            print(f"  Records: {len(self.rr_records)}")
            print(f"  Measurements: {len(self.rr_data)}")
    ...
    async def run(self):
        ...
        try:
            ...
            # 4. Get RR records
            if not await self.get_rr_records():
                print("\n⚠ No RR records found or device doesn't support respiratory rate tracking")
                await self.disconnect()
                return
            
            # 5. Optionally download detailed data for each record
            if self.rr_records:
                print(f"\nFound {len(self.rr_records)} RR records")
                download_details = input("Download detailed RR data for all records? (y/n): ").lower()
                
                if download_details == 'y':
                    for i, record in enumerate(self.rr_records, 1):
                        print(f"\n[{i}/{len(self.rr_records)}] Processing record...")
                        if await self.get_rr_data(record['utc']):
                            # Mark data with record UTC for CSV export
                            for data_point in self.rr_data:
...
async def main():
    """Entry point"""
    print("=" * 60)
    print("CL837 Respiratory Rate History Download")
    print("=" * 60)
    
    monitor = CL837RRMonitor()
    await monitor.run()
